...
# ── Runtime PATH fix for Railway/nixpacks ────────────────────────────────────
# nixpacks installs ffmpeg via Nix but the nix store bin may not be in PATH
# at gunicorn runtime. Fix this by adding all known bin paths.
...
def _fix_path():
    ...
    for d in ["/root/.nix-profile/bin", "/nix/var/nix/profiles/default/bin",
        ...
        if _os.path.isdir(d): extra.append(d)
    # Nix store ffmpeg
    for pat in ["/nix/store/*ffmpeg*/bin", "/nix/store/*-path/bin"]:
        extra.extend(d for d in _glob.glob(pat) if _os.path.isdir(d))
    if extra:
        _os.environ["PATH"] = ":".join(extra) + ":" + _os.environ.get("PATH","")
        print(f"PATH extended with {len(extra)} nix dirs")
