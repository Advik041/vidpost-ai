...
def _fix_path():
    """Find ffmpeg/yt-dlp and add to PATH. Install if missing."""
    # Step 1: Add all known bin dirs to PATH
    for d in ["/root/.nix-profile/bin", "/nix/var/nix/profiles/default/bin",
              "/run/current-system/sw/bin", "/usr/local/bin", "/usr/bin",
              "/root/.local/bin"]:
        if _os.path.isdir(d) and d not in _os.environ.get("PATH",""):
            _os.environ["PATH"] = d + ":" + _os.environ.get("PATH","")

    # Step 2: Scan nix store
    try:
        for pat in ["/nix/store/*ffmpeg*/bin"]:
            for d in _glob.glob(pat):
                if _os.path.isdir(d) and d not in _os.environ.get("PATH",""):
                    _os.environ["PATH"] = d + ":" + _os.environ.get("PATH","")
    except Exception: pass

    # Step 3: Direct nix store find
    try:
        r = _sp.run(["find", "/nix/store", "-name", "ffmpeg", "-type", "f",
                     "-path", "*/bin/ffmpeg"],
                   capture_output=True, text=True, timeout=10)
        for line in r.stdout.strip().split("\n"):
            if line and _os.path.isfile(line):
                d = _os.path.dirname(line)
                if d not in _os.environ.get("PATH",""):
                    _os.environ["PATH"] = d + ":" + _os.environ.get("PATH","")
                print(f"[startup] ffmpeg found in nix store: {line}", flush=True)
                break
    except Exception: pass

    # Step 4: Check if ffmpeg works now
    ffmpeg_ok = False
    try:
        r = _sp.run(["ffmpeg", "-version"], capture_output=True, timeout=5)
        ffmpeg_ok = r.returncode == 0
        print(f"[startup] ffmpeg: {'OK ✓' if ffmpeg_ok else 'NOT FOUND'}", flush=True)
    except FileNotFoundError:
        print("[startup] ffmpeg: NOT FOUND after PATH fix", flush=True)

    # Step 5: Install at runtime if still missing
    if not ffmpeg_ok:
        print("[startup] Installing ffmpeg via apt-get...", flush=True)
        try:
            _sp.run(["apt-get", "update", "-qq"], timeout=60, check=False)
            _sp.run(["apt-get", "install", "-y", "ffmpeg", "-qq"],
                   timeout=120, check=False)
            r2 = _sp.run(["ffmpeg", "-version"], capture_output=True, timeout=5)
            if r2.returncode == 0:
                print("[startup] ffmpeg installed via apt-get ✓", flush=True)
            else:
                print("[startup] CRITICAL: ffmpeg install failed!", flush=True)
        except Exception as e:
            print(f"[startup] apt-get failed: {e}", flush=True)

    # Step 6: Check yt-dlp
    try:
        r = _sp.run(["yt-dlp", "--version"], capture_output=True, timeout=5)
        print(f"[startup] yt-dlp: {'OK ✓' if r.returncode==0 else 'NOT FOUND'}", flush=True)
    except FileNotFoundError:
        print("[startup] yt-dlp: NOT FOUND - installing...", flush=True)
        try:
            _sp.run(["pip", "install", "-U", "yt-dlp", "-q"], timeout=60, check=False)
        except Exception: pass
...
# ── LRU Cache (replaces bare dict for TRANSCRIPT_CACHE) ──────────────────────
class LRUCache:
    """Thread-safe bounded LRU cache. Prevents unbounded memory growth."""
...
TRANSCRIPT_CACHE = LRUCache(max_size=200)  # FIX: bounded, thread-safe
...
def _cleanup_old_jobs():
    """
    Remove stale jobs from memory AND delete their /tmp directories.
    FIX: Previous version only deleted from _jobs dict, leaving /tmp dirs
         accumulating on disk. This version also cleans disk + orphaned dirs.
    """
    while True:
        time.sleep(600)
        ...
        with _jobs_lock:
            ...
            for jid in stale:
                del _jobs[jid]
        # Delete associated /tmp directories
        for jid in stale:
            job_dir = os.path.join(CLIPS_DIR, jid)
            if os.path.isdir(job_dir):
                shutil.rmtree(job_dir, ignore_errors=True)
        # Clean orphaned clip dirs (jobs lost on deploy/restart)
        try:
            for name in os.listdir(CLIPS_DIR):
                full = os.path.join(CLIPS_DIR, name)
                if os.path.isdir(full):
                    try:
                        if time.time() - os.path.getmtime(full) > 14400:
                            shutil.rmtree(full, ignore_errors=True)
                    except Exception:
                        pass
                elif name.startswith("preview_") and name.endswith(".mp4"):
                    try:
            ...
            pass
        if stale:
            print(f"Cleanup: removed {len(stale)} stale jobs + /tmp dirs")
...
def find_ffmpeg() -> str:
    """Find ffmpeg - checks system paths, nix store, and PATH."""
    import glob
    # Standard paths
    for p in ["/usr/bin/ffmpeg", "/usr/local/bin/ffmpeg",
        ...
        if os.path.exists(p):
            print(f"ffmpeg found: {p}"); return p
    # Nix store (dynamic path like /nix/store/xxx-ffmpeg-x.x/bin/ffmpeg)
    nix_paths = glob.glob("/nix/store/*ffmpeg*/bin/ffmpeg")
    if nix_paths:
        p = sorted(nix_paths)[-1]
        print(f"ffmpeg found in nix store: {p}"); return p
    # which command
    try:
        r = subprocess.run(["which","ffmpeg"], capture_output=True, text=True, timeout=5)
        if r.returncode == 0 and r.stdout.strip():
            p = r.stdout.strip()
            print(f"ffmpeg found via which: {p}"); return p
    except Exception: pass
    # Try running directly
    try:
        r = subprocess.run(["ffmpeg","-version"], capture_output=True, timeout=5)
        if r.returncode == 0:
            print("ffmpeg available as bare command"); return "ffmpeg"
    except Exception: pass
    print("WARNING: ffmpeg not found!")
    return "ffmpeg"

def find_ffprobe() -> str:
    """Find ffprobe - same logic as find_ffmpeg."""
    import glob
    for p in ["/usr/bin/ffprobe", "/usr/local/bin/ffprobe",
              "/run/current-system/sw/bin/ffprobe",
              "/root/.nix-profile/bin/ffprobe"]:
        if os.path.exists(p): return p
    nix_paths = glob.glob("/nix/store/*ffmpeg*/bin/ffprobe")
    if nix_paths: return sorted(nix_paths)[-1]
    try:
...
for _bin, _path in [("ffmpeg", FFMPEG), ("ffprobe", FFPROBE), ("yt-dlp", YTDLP)]:
    _exists = os.path.exists(_path) or _path in ("ffmpeg","ffprobe","yt-dlp")
    try:
        _r = subprocess.run([_path, "--version"], capture_output=True, timeout=5)
        _ok = _r.returncode == 0
    except Exception:
        _ok = False
    print(f"  {_bin}: path={_path} | runnable={'YES' if _ok else 'NO'}")
...
def _yt_common_args() -> list:
    ...
    FIX: Removed duplicate --extractor-args (yt-dlp only honours the last one).
         player_client, player_skip, and po_token are now merged in _build_extractor_args().
    FIX: Removed --cookies-from-browser (no Chrome on Railway - would crash yt-dlp).
    """
    args = [
        "--no-playlist",
...
def _build_extractor_args(player_client: str) -> list:
    """
    Build a single --extractor-args flag merging player_client, player_skip,
    and po_token into ONE flag value. yt-dlp silently drops all but the last
    --extractor-args if the flag appears multiple times.
    """
    parts = [f"player_client={player_client}", "player_skip=webpage,config"]
    po_token = os.environ.get("YT_PO_TOKEN", "").strip()
    if po_token:
        parts.append(f"po_token=web+{po_token}")
    return ["--extractor-args", f"youtube:{','.join(parts)}"]

def ytdlp_download(video_id: str, out_path: str, height: int = 1080) -> bool:
    """
    Download a YouTube video using a 7-strategy waterfall + Invidious fallback.

    FIX: Each strategy now uses _build_extractor_args() which merges player_client,
         player_skip, and po_token into a single --extractor-args flag. Previously,
         duplicate flags caused yt-dlp to silently drop player_skip/po_token.
    FIX: Exponential backoff between strategies (0, 2, 4, 8, 15s + jitter) to
         prevent Railway's IP from being rate-limited across all strategies at once.
    FIX: Added Invidious as strategy 7 — completely different CDN, no yt-dlp needed.
    """
    import random
    ...
    def try_strategy(player_client: str, extra: list, label: str, delay_idx: int = 0) -> bool:
        ...
        if os.path.exists(out_path):
            try:
                ...
                pass
        ext_args = _build_extractor_args(player_client)
        cmd = [YTDLP] + common + ext_args + extra + ["-o", out_path, url]
        try:
    ...
    if try_strategy("android", [
        "--user-agent", "com.google.android.youtube/19.09.37 (Linux; U; Android 14) gzip",
        "-f", fmt,
    ], "android", delay_idx=0):
        return True

    # Strategy 2: Android VR — different CDN endpoint
    if try_strategy("android_vr", [
        "--user-agent", "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36",
        "-f", "best[ext=mp4]/best",
    ], "android_vr", delay_idx=1):
        return True
    ...
    # Strategy 7: Invidious direct download — bypasses yt-dlp entirely
    INVIDIOUS_INSTANCES = [
    ...
    for inst in INVIDIOUS_INSTANCES:
        try:
            r = requests.get(f"{inst}/api/v1/videos/{video_id}", timeout=8,
                             headers={"User-Agent": "Mozilla/5.0"})
            if not r.ok:
                continue
            data = r.json()
            for fmt_data in data.get("formatStreams", []):
                if "video/mp4" in fmt_data.get("type", "") and fmt_data.get("url"):
                    with requests.get(fmt_data["url"], stream=True, timeout=300) as dl:
                        dl.raise_for_status()
                        with open(out_path, "wb") as f:
                            for chunk in dl.iter_content(1024 * 1024):
                                if chunk:
                                    f.write(chunk)
                    if os.path.exists(out_path) and os.path.getsize(out_path) > 10000:
                        print(f"Invidious OK ({inst}): {os.path.getsize(out_path)//1024}KB")
                        return True
        except Exception as e:
...
def yt_stream_url():
    """
    Get YouTube direct MP4 URL using professional download APIs with residential IPs.
    
    Strategy waterfall (each uses different IP pools):
      1. YT-DLP via API service (ytdlp-api on Railway — same server, different approach)  
      2. RapidAPI "YouTube MP4" (residential proxies, most reliable, needs key)
      3. RapidAPI "YouTube Video and Shorts Downloader" (backup)
      4. Cobalt.tools (open source, no key)
      5. Y2mate API (widely used)
      6. Invidious + Piped (6+ instances)
    """
    ...
    if RAPIDAPI_KEY:
        try:
            r = requests.get(
                "https://youtube-mp4s.p.rapidapi.com/youtube-mp4",
                params={"url": yt_url, "quality": quality + "p"},
                headers={"X-RapidAPI-Key": RAPIDAPI_KEY,
                         "X-RapidAPI-Host": "youtube-mp4s.p.rapidapi.com"},
                timeout=20)
            if r.ok:
                d = r.json()
                link = d.get("url") or d.get("link") or d.get("downloadUrl")
                if link:
                    log("rapidapi-mp4s", "OK")
                    return jsonify({"stream_url":link,"video_id":vid,"title":title,"method":"rapidapi_mp4s"})
        except Exception as e: log("rapidapi-mp4s", e)

    # ── 2. RapidAPI: yt-to-mp4 ───────────────────────────────────────────────
    if RAPIDAPI_KEY:
        try:
            r = requests.get(
                "https://yt-to-mp4.p.rapidapi.com/",
                params={"url": yt_url},
                headers={"X-RapidAPI-Key": RAPIDAPI_KEY,
                         "X-RapidAPI-Host": "yt-to-mp4.p.rapidapi.com"},
                timeout=20)
            if r.ok:
                d = r.json()
                link = d.get("url") or d.get("link") or d.get("mp4")
                if link:
                    log("rapidapi-yt-to-mp4", "OK")
                    return jsonify({"stream_url":link,"video_id":vid,"title":title,"method":"rapidapi_yt_to_mp4"})
        except Exception as e: log("rapidapi-yt-to-mp4", e)

    # ── 3. RapidAPI: youtube-downloader-api2 ─────────────────────────────────
    if RAPIDAPI_KEY:
        try:
            r = requests.post(
                "https://youtube-downloader-api2.p.rapidapi.com/mp4",
                json={"url": yt_url, "quality": quality + "p"},
                headers={"X-RapidAPI-Key": RAPIDAPI_KEY,
                         "X-RapidAPI-Host": "youtube-downloader-api2.p.rapidapi.com",
                         "Content-Type": "application/json"},
                timeout=20)
            if r.ok:
                d = r.json()
    ...
    try:
        ...
        if r1.ok:
            d1 = r1.json()
            vid_key = None
            for fmt_data in d1.get("links",{}).get("mp4",{}).values():
                q_str = fmt_data.get("q", "0").replace("p", "").strip()
                try:
                    q_int = int(q_str)
                except ValueError:
                    q_int = 0
                if q_int <= int(quality):
                    vid_key = fmt_data.get("k"); break
            if vid_key:
                r2 = requests.post("https://www.y2mate.com/mates/convertV2/index",
                    data={"vid":vid,"k":vid_key},
                    headers={"User-Agent":"Mozilla/5.0","X-Requested-With":"XMLHttpRequest"},
                    timeout=15)
                if r2.ok:
                    d2 = r2.json()
                    ...
                    if link:
                        log("y2mate","OK")
                        return jsonify({"stream_url":link,"video_id":vid,"title":title,"method":"y2mate"})
    except Exception as e: log("y2mate", e)
    ...
    invidious = [
        ...
        "https://invidious.fdn.fr","https://invidious.io",
    ]
    for inst in invidious:
        try:
            ...
            if r.ok:
                d = r.json()
                # formatStreams = muxed (video+audio) — best for direct use
                for f in d.get("formatStreams",[]):
                    if "video/mp4" in f.get("type","") and f.get("url"):
                        log("invidious",f"OK ({inst})")
                        return jsonify({"stream_url":f["url"],"video_id":vid,"title":title,
                                       "method":f"invidious","quality":f.get("resolution","?")})
        except Exception as e: log(f"invidious({inst})",e)

    # ── 7. Piped instances ───────────────────────────────────────────────────
    piped = [
        "https://pipedapi.kavin.rocks","https://pipedapi.adminforge.de",
        "https://piped-api.garudalinux.org","https://api.piped.projectsegfau.lt",
    ]
    for inst in piped:
        try:
            ...
            if r.ok:
                ...
                for s in d.get("videoStreams",[]):
                    if "mp4" in s.get("mimeType","") and s.get("height",999)<=int(quality) and s.get("url"):
                        log("piped",f"OK ({inst})")
                        return jsonify({"stream_url":s["url"],"video_id":vid,"title":title,"method":"piped"})
        except Exception as e: log(f"piped({inst})",e)

    # ── 8. yt-dlp if available ───────────────────────────────────────────────
    try:
        r = subprocess.run(
            [YTDLP,"--get-url","-f",f"best[height<={quality}][ext=mp4]/best[ext=mp4]/best",
             "--no-playlist","--socket-timeout","15",yt_url],
            capture_output=True, text=True, timeout=25)
        if r.returncode==0 and r.stdout.strip():
            link = r.stdout.strip().split("\n")[0]
            if link.startswith("http"):
                log("ytdlp-get-url","OK")
                return jsonify({"stream_url":link,"video_id":vid,"title":title,"method":"ytdlp"})
    except Exception as e: log("ytdlp",e)

    log("ALL","failed")
    return jsonify({"error":"All download services unavailable","video_id":vid,
                    "title":title,"show_upload_prompt":True}), 422
...
def twitter_auth():
    ...
    # FIX: encode challenge into state param (format: user_id:nonce:challenge)
    # The old approach used session["twitter_challenge"] which breaks with
    # multiple Gunicorn workers (callback may hit a different worker than auth).
    state = f"{user_id}:{nonce}:{challenge}"
    params = {
        "response_type": "code",
...
def twitter_callback():
    ...
    # FIX: recover challenge from state — no session lookup needed
    parts = state.split(":", 2)
    if len(parts) != 3:
        return redirect(f"{FRONTEND_URL}?error=twitter_invalid_state")
    user_id, nonce, challenge = parts
    resp = requests.post(
...
def create_clip():
    ...
    stream_url = data.get("streamUrl", "").strip()  # Direct CDN URL from YTStream
    start  = float(data.get("start", 0))
    end    = float(data.get("end", 60))
    formats= data.get("formats", ["vertical", "horizontal"])
    user_id= data.get("user_id", "")

    if not video_id and not upload_path and not stream_url:
    ...
    t = threading.Thread(target=process_clip_job,
                         args=(job_id, video_id, upload_path, start, end, formats, user_id, stream_url))
    t.daemon = True
    t.start()
    return jsonify({"jobId": job_id})


def process_clip_job(job_id, video_id, upload_path, start, end, formats, user_id="", stream_url=""):
    ...
    try:
        if upload_path and os.path.exists(upload_path):
            ...
            if not success:
                raise Exception("YouTube download failed — use Upload tab instead.")
        else:
...
@app.route("/proxy-download", methods=["POST", "OPTIONS"])
def proxy_download():
    """
    Server-side proxy to download a video URL and stream it back to browser.
    Used when browser CORS blocks direct CDN fetch.
    The browser gets the video as a blob, then uploads to /analyse-upload.
    """
    if request.method == "OPTIONS":
        return jsonify({}), 200
    ...
    # Security: only allow known video CDN domains
    allowed = [
        "googlevideo.com","ytimg.com","youtube.com",
        "cobalt.tools","invidious","piped","y2mate",
        "ssyoutube","savefrom","rapidapi",
        "akamaized.net","cloudfront.net","fastly.net",
    ]
    from urllib.parse import urlparse
    host = urlparse(url).netloc.lower()
    if not any(a in host for a in allowed):
        # Allow anyway but log it — video CDNs use many domains
        print(f"proxy-download: non-allowlisted host {host}")
    ...
    try:
        ...
        resp = Response(generate(), status=upstream.status_code,
                       mimetype=content_type, direct_passthrough=True)
        resp.headers["Content-Disposition"] = "attachment; filename=video.mp4"
        if content_length:
...
def post_youtube_short(token_data, text, video_url=""):
    ...
    meta = {
        "snippet": {"title": title, "description": text, "tags": ["shorts"], "categoryId": "22"},
        "status": {"privacyStatus": "public", "selfDeclaredMadeForKids": False},
    }

    # FIX: Use resumable upload — streams video in 5 MB chunks, never loads into RAM.
    # The old approach used requests.get(video_url).content which loaded the entire
    # file into memory, OOM-killing Railway on clips > ~100 MB.
    ...
    if not init_resp.ok:
        raise Exception(f"YouTube resumable init failed: {init_resp.text[:300]}")

    upload_url = init_resp.headers.get("Location")
    if not upload_url:
        raise Exception("YouTube did not return upload Location URL")

    CHUNK_SZ = 5 * 1024 * 1024  # 5 MB — YouTube minimum chunk size
    offset = 0
    resp = None
    with requests.get(video_url, stream=True, timeout=300) as video_resp:
        video_resp.raise_for_status()
        for chunk in video_resp.iter_content(chunk_size=CHUNK_SZ):
            if not chunk:
                continue
            ...
            resp = requests.put(
                upload_url,
                headers={
                    "Content-Range": f"bytes {offset}-{chunk_end}/*",
            ...
            if resp.status_code not in (200, 201, 308):
                raise Exception(f"YouTube chunk upload failed at byte {offset}: {resp.status_code} {resp.text[:200]}")
            offset += len(chunk)

    if not resp or resp.status_code not in (200, 201):
        raise Exception("YouTube upload did not complete")
...
def post_snapchat(token_data, text, video_url=""):
    ...
    with requests.get(video_url, stream=True, timeout=300) as video_resp:
        ...
        upload_resp = requests.post(
            ...
            headers={"Authorization": f"Bearer {access_token}"},
            files={"file": ("video.mp4", video_resp.raw, "video/mp4")},
            data={"name": text[:50], "type": "VIDEO"},
        )
    if not upload_resp.ok:
        raise Exception(f"Snapchat upload failed: {upload_resp.text[:300]}")
    return {"id": upload_resp.json().get("media", {}).get("id", ""),
            "platform": "snapchat", "status": "posted"}
...
def _refresh_google_token(token_data: dict) -> str:
    """Refresh a Google/YouTube access token using the stored refresh token."""
    refresh_token = token_data.get("refresh_token", "")
...
@app.route("/youtube/my-videos", methods=["GET", "OPTIONS"])
def youtube_my_videos():
    """Phase 3: List the authenticated user's own YouTube videos for import."""
    if request.method == "OPTIONS":
        return jsonify({}), 200
    user_id = request.args.get("user_id", "")
    if not user_id:
        return jsonify({"error": "Missing user_id"}), 400
    token_data = get_token(user_id, "youtube")
    if not token_data:
        return jsonify({"error": "YouTube not connected"}), 401
    access_token = _refresh_google_token(token_data)
    try:
        resp = requests.get(
        ...
        if not resp.ok:
            return jsonify({"error": f"YouTube API error: {resp.status_code}"}), resp.status_code
...
@app.route("/import/gdrive", methods=["POST", "OPTIONS"])
def import_gdrive():
    """Phase 3: Import a video from Google Drive using the user's Google OAuth token."""
    if request.method == "OPTIONS":
        return jsonify({}), 200
    data = request.get_json() or {}
    user_id = data.get("user_id", "")
    ...
    if not meta_resp.ok:
        return jsonify({"error": f"Google Drive: {meta_resp.status_code}"}), 400
    meta = meta_resp.json()
    filename = meta.get("name", "gdrive_video.mp4")
    if "video" not in meta.get("mimeType", ""):
        return jsonify({"error": f"File is not a video (type: {meta.get('mimeType')})"}), 400
    upload_id = str(uuid.uuid4())[:8]
    upload_path = os.path.join(UPLOADS_DIR, f"{upload_id}.mp4")
    with requests.get(
        ...
        if not dl_resp.ok:
            return jsonify({"error": "Failed to download from Google Drive"}), 502
        with open(upload_path, "wb") as f:
            for chunk in dl_resp.iter_content(1024 * 1024):
                if chunk:
                    f.write(chunk)
    if not os.path.exists(upload_path) or os.path.getsize(upload_path) < 10000:
        return jsonify({"error": "Downloaded file too small or empty"}), 502
    duration, title = 600, os.path.splitext(filename)[0]
    try:
        probe = subprocess.run(
            [FFPROBE, "-v", "quiet", "-print_format", "json", "-show_format", upload_path],
            capture_output=True, text=True, timeout=30)
        ...
        pass
    clips = _detect_clips(title, duration, f"Imported from Google Drive: {title}. Duration: {duration}s.", GROQ_KEY)
    with _upload_lock:
        UPLOAD_STORE[upload_id] = upload_path
    threading.Timer(7200, lambda: _cleanup_upload(upload_id, upload_path)).start()
    return jsonify({"uploadId": upload_id, "uploadPath": upload_path, "title": title,
                    "duration": duration, "clips": clips, "transcriptLength": 0,
                    "mode": "gdrive", "source": f"Google Drive: {filename}"})
