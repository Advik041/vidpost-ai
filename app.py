...
def _fix_path():
    """Add nix store bin dirs to PATH so ffmpeg/yt-dlp are found at runtime."""
    extra = []
    for d in ["/root/.nix-profile/bin", "/nix/var/nix/profiles/default/bin",
        ...
        if _os.path.isdir(d) and d not in _os.environ.get("PATH",""):
            extra.append(d)
    # Scan nix store for ffmpeg
    try:
        for pat in ["/nix/store/*ffmpeg*/bin", "/nix/store/*-path/bin"]:
            for d in _glob.glob(pat):
                if _os.path.isdir(d) and d not in _os.environ.get("PATH",""):
                    extra.append(d)
    except Exception: pass
    if extra:
        _os.environ["PATH"] = ":".join(extra) + ":" + _os.environ.get("PATH","")
        print(f"[startup] PATH extended: {extra}", flush=True)
    # Verify ffmpeg
    try:
        r = _sp.run(["ffmpeg", "-version"], capture_output=True, timeout=5)
        print(f"[startup] ffmpeg: {'OK' if r.returncode==0 else 'NOT FOUND'}", flush=True)
    except FileNotFoundError:
        print("[startup] ffmpeg: NOT FOUND - trying nix store direct", flush=True)
        # Last resort: find ffmpeg binary directly
        try:
            result = _sp.run(["find", "/nix/store", "-name", "ffmpeg", "-type", "f"],
                           capture_output=True, text=True, timeout=10)
            bins = [b for b in result.stdout.strip().split("\n") if b and "bin/ffmpeg" in b]
            if bins:
                ffmpeg_dir = _os.path.dirname(bins[0])
                _os.environ["PATH"] = ffmpeg_dir + ":" + _os.environ.get("PATH","")
                print(f"[startup] ffmpeg found via find: {bins[0]}", flush=True)
        except Exception as fe:
            print(f"[startup] find ffmpeg failed: {fe}", flush=True)
